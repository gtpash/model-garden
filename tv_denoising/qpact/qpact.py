...
from models import qPACT, PACTMisfitForm, circularInclusion, rprint
...
noisy_data = dl.project(u_fun_true*ufl.exp(m_fun_true.sub(1)), qpact.Vh[hp.STATE])
...
# set up the misfit
misfit_form = PACTMisfitForm(noisy_data, dl.Constant(NOISE_VARIANCE))
...
obs_fun = dl.project(ug_fun*mg_fun.sub(1), qpact.Vh[hp.STATE])
...
obstv_fun = dl.project(utv_fun*mtv_fun.sub(1), qpact.Vh[hp.STATE])
